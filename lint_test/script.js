...
const url = 'https://veff2022-h1.herokuapp.com/api/v1/tunes';
const timing = 0;
let difference = 0;
let down = false;
let playing = false;
...
function press_button(e) {
  ...
  if (getNote(e.key) !== undefined) {
    if (activeTextarea.id != 'recordName') {
      const element = document.getElementById(getNote(e.key));
...
function playTune() {
  ...
  for (let i=0; i<selected_tune.length; i++) {
    const now = Tone.now();
...
function press_button_for_playback(note) {
  ...
  const element = document.getElementById(getNote(key));
  element.classList.add('press-button');
}
